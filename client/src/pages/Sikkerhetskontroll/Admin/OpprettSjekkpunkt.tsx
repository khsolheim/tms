...
const OpprettSjekkpunkt: React.FC = () => {
  ...
  const [lasterSystemer, setLasterSystemer] = useState(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/sjekkpunkt', sjekkpunktData);
      
      // Navigate back to admin page on success
      navigate('/sikkerhetskontroll');
      
    } catch (error: any) {
      console.error('Feil ved lagring av sjekkpunkt:', error);
      setFeil(error.response?.data?.error || 'Kunne ikke lagre sjekkpunkt');
    } finally {
      setLagrer(false);
    }
  };
    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto px-2 py-1">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-[#003366]">Opprett nytt sjekkpunkt</h1>
          <p className="text-gray-600 mt-2">Definer et nytt sjekkpunkt som kan brukes i sikkerhetskontroller</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="cards-spacing-vertical">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 cards-spacing-grid">
              ...
              <div>
                ...
                <select 
                  ...
                  onChange={handleChange} 
                  required 
                  disabled={lasterSystemer}
                  className="w-full border border-gray-300 rounded-lg px-2 py-1 focus:ring-2 focus:ring-[#003366] focus:border-transparent disabled:opacity-50"
                >
                  <option value="">{lasterSystemer ? 'Laster systemer...' : 'Velg system...'}</option>
          ...
          <SectionCard icon={FaImage} title="Bilder og videoer">
            <div className="grid grid-cols-1 lg:grid-cols-2 cards-spacing-grid">
              <div>
                <label className="block font-semibold mb-2 text-gray-700">Instruksjonsbilde</label>
                <input 
                  type="file" 
                  name="bilde" 
                  accept="image/*" 
                  ...
                  className="w-full border border-gray-300 rounded-lg px-2 py-1 focus:ring-2 focus:ring-[#003366] focus:border-transparent" 
                />
                <p className="text-sm text-gray-500 mt-1">Last opp bilde som viser hvordan kontrollen utføres</p>
              </div>
              
              <div>
                <label className="block font-semibold mb-2 text-gray-700">Instruksjonsvideo</label>
                <input 
                  type="file" 
                  name="video" 
                  accept="video/*" 
                  ...
                  className="w-full border border-gray-300 rounded-lg px-2 py-1 focus:ring-2 focus:ring-[#003366] focus:border-transparent" 
                />
                <p className="text-sm text-gray-500 mt-1">Last opp video som demonstrerer kontrollen</p>
              </div>
            </div>
          </SectionCard>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 cards-spacing-grid">
            ...
            <SectionCard icon={FaUsers} title="Førerkortklasser">
              {lasterKlasser || lasterSystemer ? (
                <p className="text-gray-500">Laster tilgjengelige data...</p>
              ) : (
                <>
          ...
          <div className="flex justify-end pt-6 border-t border-gray-200">
            ...
            <div className="flex cards-spacing-grid">
              ...
              >
                {lagrer ? 'Lagrer...' : 'Lagre sjekkpunkt'}
              </button>
            </div>
          </div>
        </form>
...
export default OpprettSjekkpunkt; 