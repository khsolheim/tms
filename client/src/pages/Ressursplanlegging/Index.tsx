...
import ressursplanleggingService, { RessursplanleggingDashboardData, ResourceModule, ResourceMetric } from '../../services/ressursplanlegging.service';
...
export default function RessursplanleggingIndex() {
  const [hoveredCard, setHoveredCard] = useState<string | null>(null);
  const [dashboardData, setDashboardData] = useState<RessursplanleggingDashboardData | null>(null);
  ...
  const hentDashboardData = async () => {
    try {
      const data = await ressursplanleggingService.hentDashboardData();
      setDashboardData(data);
    } catch (error) {
      console.error('Feil ved henting av ressursplanlegging dashboard data:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!dashboardData) {
    return <div className="p-6">Kunne ikke laste ressursplanleggingsdata</div>;
  }

  const { resourceMetrics, resourceModules, quickActions } = dashboardData;
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-lg text-gray-600">
              Optimaliser bruken av kjøretøy, personale og utstyr for maksimal effektivitet
            </p>
          </div>
          <div className="flex items-center space-x-4">
            <Link
              to="/ressursplanlegging/booking/ny"
            ...
            <Link
              to="/ressursplanlegging/kapasitet/rapport"
