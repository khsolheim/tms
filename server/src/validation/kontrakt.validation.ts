...
const personSchema = z.object({
  fornavn: z.string().min(1, 'Fornavn er pakrevd').max(100, 'Fornavn kan ikke være lengre enn 100 tegn'),
  etternavn: z.string().min(1, 'Etternavn er pakrevd').max(100, 'Etternavn kan ikke være lengre enn 100 tegn'),
  ...
  gate: z.string().min(1, 'Gateadresse er pakrevd').max(200, 'Gateadresse kan ikke være lengre enn 200 tegn'),
  postnr: z.string().regex(postnummerRegex, 'Postnummer må være 4 siffer'),
  poststed: z.string().min(1, 'Poststed er pakrevd').max(100, 'Poststed kan ikke være lengre enn 100 tegn')
});

// Kontrakt opprettelse schema (kun body data)
const kontraktOpprettBodySchema = z.object({
  // Elevinfo
  elevFornavn: z.string().min(1, 'Fornavn er pakrevd').max(100),
  elevEtternavn: z.string().min(1, 'Etternavn er pakrevd').max(100),
  ...
  elevGate: z.string().min(1, 'Gateadresse er pakrevd').max(200),
  elevPostnr: z.string().regex(postnummerRegex, 'Postnummer må være 4 siffer'),
  elevPoststed: z.string().min(1, 'Poststed er pakrevd').max(100),
...
export const kontraktOppdaterSchema = z.object({
  ...
  body: z.object({
    ...
    elevFornavn: z.string().min(1, 'Fornavn er pakrevd').max(100).optional(),
    elevEtternavn: z.string().min(1, 'Etternavn er pakrevd').max(100).optional(),
    ...
    elevGate: z.string().min(1, 'Gateadresse er pakrevd').max(200).optional(),
    elevPostnr: z.string().regex(postnummerRegex, 'Postnummer må være 4 siffer').optional(),
    elevPoststed: z.string().min(1, 'Poststed er pakrevd').max(100).optional(),
