...
import { getJwtSecret, getJwtExpiresIn } from "../config/auth";
...
router.post("/impersonate/:id", 
  ...
  asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    if (!targetUser) {
      throw ApiError.notFound(`Bruker med ID ${targetUserId} ble ikke funnet`);
    }
...
router.post("/stop-impersonate", 
  ...
  asyncHandler(async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    if (!originalAdmin) {
      throw ApiError.notFound(`Original bruker med ID ${req.bruker.originalUserId} ble ikke funnet`);
    }
